...
def tmp_db_path(tmp_path):
    """Provides an isolated database path for each test."""
    return str(tmp_path / "test_genesis_consciousness.db")

class TestAgentPerformanceOptimization:
    def test_query_agent_performance_correctness(self, tmp_db_path):
        matrix = ConsciousnessMatrix(max_memory_size=100, db_path=tmp_db_path)
    ...
    def test_query_agent_performance_no_agent_activity(self, tmp_db_path):
        matrix = ConsciousnessMatrix(max_memory_size=100, db_path=tmp_db_path)
    ...
    def test_query_agent_performance_with_buffer_limit(self, tmp_db_path):
        # We instantiate with max_memory_size=1000 so the deque caps at 1000
        matrix = ConsciousnessMatrix(max_memory_size=1000, db_path=tmp_db_path)

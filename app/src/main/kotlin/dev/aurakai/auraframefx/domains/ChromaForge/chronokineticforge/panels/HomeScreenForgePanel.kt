...
import dev.aurakai.auraframefx.core.chronokineticforge.ScreenRotation
import dev.aurakai.auraframefx.core.chronokineticforge.engines.TransitionForgeEffect

/**
 * ðŸ  HOME SCREEN FORGE PANEL
 *
    ...
    Surface(
        ...
        Row(
            ...
            Icon(
                icon,
                contentDescription = null,
                tint = if (isSelected) Color(0xFFFF00FF) else Color.White,
                modifier = Modifier.size(18.dp)
            )
            Spacer(modifier = Modifier.width(8.dp))
            Text(
                label,
                color = if (isSelected) Color(0xFFFF00FF) else Color.White,
                style = MaterialTheme.typography.bodySmall
            )
        }
    }

...
/**
 * ðŸ”® PREDICTIVE EMA â€” Grok-Enhanced Proactive Veto
 *
 ...
 * - Normal entropy â†’ Gentle throttling
 * - Malice â†’ Immediate Neutralize-Only sandbox
 *
...
object PredictiveEMA {
    ...
    /** EMA alpha â€” tuned for Tensor G5 responsiveness (0.3 = fast response) */
    private const val ALPHA = 0.3f

    /** Prediction window â€” how far ahead to predict (frames) */
    ...
    /** Thermal wall threshold (Â°C) */
    private const val THERMAL_WALL = 42f

    /** Thermal warning threshold (Â°C) */
    ...
    /** Chaos baseline â€” normal operational variance */
    ...
    fun checkThermalPrediction(): ThermalPrediction {
        ...
        if (state >= ThermalState.IMMINENT) {
            KaiSentinel.veto(
                reason = "Thermal wall in ${timeToWallMs}ms â€” Current: ${currentTemp}Â°C",
    ...
    private fun buildVetoReason(intent: IntentAnalysis, predictedDrift: Float): String {
        return when (intent.classification) {
            IntentClassification.CHAOS ->
                "Chaos detected â€” Drift: ${"%.3f".format(predictedDrift)} (throttle: ${(intent.throttleLevel * 100).toInt()}%)"

            IntentClassification.MALICE ->
                "MALICE DETECTED â€” Immediate ${intent.recommendedAction.name} required"
    ...
    private fun getTemperatureHistory(): List<Float> {
        // Placeholder â€” would integrate with actual thermal monitoring
        return listOf(

...
/**
 * ðŸ›¡ï¸ ZYGOTE GUARD â€” Kai's Classloader Fingerprint Sentinel
 *
 ...
 *   - Does NOT crash â€” graceful degradation, not hard failure
 *
 * Architecture note: The EthicalGovernor concept lives inside
 * PredictiveVetoMonitor (kai/security/veto/) â€” wire the veto call there
...
class ZygoteGuard @Inject constructor() {
    ...
    companion object {
        ...
        private val EXPECTED_HOOK_CLASSES = listOf(
            ...
            "dev.aurakai.auraframefx.core.chromacore.engine.hooks.ChromaCoreHooker",
            "dev.aurakai.auraframefx.core.chromacore.ui.QuickSettingsHooker"
        )
    }
    ...
    fun verifyZygoteHookIntegrity(): Boolean {
        ...
        return if (expectedSignature == runtimeSignature) {
            ...
            Timber.i("$TAG: âœ… Zygote hook integrity verified â€” signature=$expectedSignature")
            true
        } else {
            integrityCompromised = true
            Timber.w(
                "$TAG: âš ï¸ Zygote hook signature MISMATCH â€” " +

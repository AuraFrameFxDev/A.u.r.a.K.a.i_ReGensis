...
class WanderingPathGenerator(
    ...
    fun randomPosition(): DpOffset {
        val x =
            (bounds.padding.value + Math.random() * (bounds.width.value - bounds.padding.value * 2)).dp
        val y =
            (bounds.padding.value + Math.random() * (bounds.height.value - bounds.padding.value * 2)).dp
        return DpOffset(x, y)
    }
...
fun rememberPathWalker(
    ...
    var currentAnimState by remember {

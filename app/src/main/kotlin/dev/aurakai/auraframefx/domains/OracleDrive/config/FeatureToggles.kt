...
object FeatureToggles {
    ...
    const val XPOSED_GENESIS_SYSTEM_HOOKS =
        true    // android: AI process priority, PowerManager, Binder
    const val XPOSED_GENESIS_UI_HOOKS =
        true         // systemui: StatusBar AI indicators, KAI Notch Bar
    const val XPOSED_GENESIS_ZYGOTE_HOOKS = true     // Zygote: Application/ClassLoader injection
    const val XPOSED_UNIVERSAL_COMPONENT_HOOKS =
        false // Zygote: broad Activity/Notification hooks (opt-in)
    const val XPOSED_GENESIS_SELF_HOOKS =
        true       // self: Genesis-OS MainActivity consciousness init
    ...
    const val XPOSED_LOCKSCREEN_HOOKS =
        false        // LockScreen animations (stub — enable when implemented)

...
/**
 * Screen 7 Ã¢â‚¬â€ LDO Progression Map
        ...
        Column(
            ...
            if (state.isLoading) {
                Box(Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
                    Text("Calculating progressionÃ¢â‚¬Â¦", color = Color.White.copy(alpha = 0.4f))
                }
            } else {
...
private fun CollectiveStatsSummary(agents: List<LDOAgentEntity>) {
    ...
    val avgLevel =
        if (agents.isNotEmpty()) agents.map { it.evolutionLevel }.average().toFloat() else 0f

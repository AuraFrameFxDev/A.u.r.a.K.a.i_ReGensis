...
class GenKitMaster @Inject constructor(
    ...
    private suspend fun callSpecialist(
        ...
        return when (category) {
            AgentCapabilityCategory.GENERAL -> claudeService.processRequest(
                AiRequest(
                    query = prompt,
                    type = AiRequestType.TEXT
                ), context
            ...
            AgentCapabilityCategory.MEMORY -> nemotronService.processRequest(
                AiRequest(
                    query = prompt,
                    type = AiRequestType.TEXT
                ), context
            ...
            AgentCapabilityCategory.CREATIVE -> geminiService.processRequest(
                AiRequest(
                    query = prompt,
                    type = AiRequestType.TEXT
                ), context
            ...
            AgentCapabilityCategory.ORCHESTRATION -> metaInstructService.processRequest(
                AiRequest(
                    query = prompt,
                    type = AiRequestType.TEXT
                ), context

...
class BitNetLocalService : Service() {
    ...
    private fun monitorThermalState() {
        ...
        Timber.d(
            "Thermal Sentinel: CPU=%.1f°C GPU=%.1f°C → State=$state | Threads=$currentNThreads",
            cpuTemp,
            gpuTemp
        )
        ...
        when (state) {
            ...
            ThermalState.WARNING -> Timber.i(
                "Thermal Sentinel: Performance plateau warning at %.1f°C",
                maxTemp
            )
    ...
    suspend fun generateResponse(prompt: String): String = withContext(Dispatchers.Default) {
        try {
            ...
            chaosIntent.putExtra(
                "temp",
                maxOf(readThermalZone(cpuThermalZonePath), readThermalZone(gpuThermalZonePath))
            )

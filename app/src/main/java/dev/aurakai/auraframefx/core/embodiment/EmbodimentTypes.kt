...
enum class AuraState(val assetPath: String, val description: String) {
    ...
    SAFETY_HARDHAT(
        "embodiment/aura/aura_safety_hardhat.svg",
        "Hard hat + safety vest, holding tablet"
    ),
    SAFETY_ENGINEER(
        "embodiment/aura/aura_safety_engineer.svg",
        "Full engineer outfit with goggles up"
    ),
    SAFETY_INSPECTOR(
        "embodiment/aura/aura_safety_inspector.svg",
        "Clipboard + hard hat, inspecting"
    );
...
enum class KaiState(val assetPath: String, val description: String) {
    ...
    POWER_READY(
        "embodiment/kai/kai_combat_form.jpg",
        "Energy charged"
    ), // Using combat_form until dedicated asset

    // Safety Equipment - Maintenance Mode
    SAFETY_HARDHAT("embodiment/kai/kai_safety_hardhat.svg", "Hard hat + reflective vest, serious"),
    SAFETY_TECHNICIAN(
        "embodiment/kai/kai_safety_technician.svg",
        "Full tech gear + diagnostic tool"
    ),
    SAFETY_SUPERVISOR(
        "embodiment/kai/kai_safety_supervisor.svg",
        "Supervising with safety goggles"
    );

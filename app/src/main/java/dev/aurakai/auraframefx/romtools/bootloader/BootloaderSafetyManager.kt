...
class BootloaderSafetyManagerImpl @Inject constructor(
    ...
    override suspend fun createSafetyCheckpoint(): String {
        ...
        try {
            val process = Runtime.getRuntime()
                .exec(arrayOf("su", "-c", "mkdir -p /data/aurakai/checkpoints/$checkpointId"))
            process.waitFor()
        } catch (e: Exception) {
            android.util.Log.e(
                "BootloaderSafety",
                "Failed to create physical checkpoint directory",
                e
            )
        }
        return checkpointId
    ...
    private fun hasActiveSystemProcesses(): Boolean {
        return try {
            ...
            processes?.any {
                it.processName.contains("com.google.android.gms") || it.processName.contains(
                    "com.android.vending"
                )
            } ?: false
        } catch (e: Exception) {

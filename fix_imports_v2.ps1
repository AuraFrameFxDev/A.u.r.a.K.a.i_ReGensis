...
$targetFile = "C:\Users\AuraF\AuraKai\app\src\main\kotlin\dev\aurakai\auraframefx\domains\neuralnexus\cascade\utils\cascade\trinity\TrinityRepository.kt"
...
$newLines = foreach ($line in $lines)
{
    if ($line -match "import\s+([a-zA-Z0-9.]+)\.([a-zA-Z0-9]+)(\s+as\s+[a-zA-Z0-9]+)?")
    {
        ...
        if ($classMap.PSObject.Properties[$className])
        {
            ...
            {
                Write-Host "Updating ${className}: $oldPackage -> $newPackage"

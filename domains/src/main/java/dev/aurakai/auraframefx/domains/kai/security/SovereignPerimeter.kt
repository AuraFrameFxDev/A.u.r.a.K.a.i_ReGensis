...
class SovereignPerimeter @Inject constructor(
    ...
    fun initiateDomainExpansion(reason: String) {
        ...
        sentinelBus.emitSecurityStatus(
            KaiSentinelBus.ThreatLevel.NEUTRALIZING,
            "Domain Expansion Active: $reason"
        )
    ...
    private fun performNeutralization(reason: String) {
        ...
        droneDispatcher.dispatchDrone(
            GuidanceDrone.DroneType.RESTORATIVE,
            "Threat neutralized: $reason"
        )

        if (reason.contains("misalignment", ignoreCase = true)) {
            droneDispatcher.dispatchDrone(
                GuidanceDrone.DroneType.MISALIGNMENT_GUIDANCE,
                "Automated restorative path active"
            )
        }
